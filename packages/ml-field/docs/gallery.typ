...
#import "@local/ml-optim:0.1.0" as opt   // descent trajectories to overlay
...
path (from `ml-optim`) is overlaid in the same coordinates.

== contour — a loss bowl with a *computed* gradient-descent path and marked minimum
// the path is the real GD iteration from ml-optim, not hand-typed points
#let bowl-grad(p) = (2.0 * p.at(0), 6.0 * p.at(1))
#contour((x, y) => x * x + 3.0 * y * y, xlim: (-3, 3), ylim: (-2, 2),
  paths: (opt.gd(bowl-grad, (-2.6, 1.6), lr: 0.12, steps: 12),),
...
#stack(dir: ltr, spacing: 8mm,
  contour((x, y) => x * x + 100.0 * y * y, xlim: (-2.6, 2.6), ylim: (-0.55, 0.55),
    ...
    paths: (opt.gd(rav, (-2.3, 0.45), lr: 0.0090, steps: 34),),
    marks: ((0, 0, [min]),), title: [plain GD]),
  contour((x, y) => x * x + 100.0 * y * y, xlim: (-2.6, 2.6), ylim: (-0.55, 0.55),
    samples: 60, levels: 10, size: (62mm, 24mm),
    paths: (opt.momentum(rav, (-2.3, 0.45), lr: 0.0035, steps: 34),),
