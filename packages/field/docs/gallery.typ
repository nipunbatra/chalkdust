// field gallery — 2-D and 3-D fields of f(x, y).
...
path (from `optim`) is overlaid in the same coordinates.

== contour — a loss bowl with a *computed* gradient-descent path and marked minimum
// the path is the real GD iteration from optim, not hand-typed points
